...
async function cardsForUsers(userIds){
 const ids=[...new Set((userIds||[]).map(Number).filter(Number.isInteger).filter(x=>x>0))];if(!ids.length)return new Map();const idsJson=JSON.stringify(ids);
 const rows=await sql`SELECT uc.user_id,uc.type,uc.text,uc.matches_used,uc.times_won,uc.times_seen,uc.is_player_card,uc.owned,(EXISTS(SELECT 1 FROM card_origins co WHERE co.creator_user_id=uc.user_id AND co.type=uc.type AND co.text_key=lower(regexp_replace(trim(uc.text),'\\s+',' ','g'))) OR EXISTS(SELECT 1 FROM canonical_cards cc JOIN canonical_card_authors ca ON ca.canonical_card_id=cc.id WHERE ca.user_id=uc.user_id AND cc.card_type=CASE WHEN uc.type='whiteCards' THEN 'white' ELSE 'black' END AND cc.normalized_text=lower(regexp_replace(trim(uc.text),'\\s+',' ','g')))) AS is_authored FROM user_cards uc WHERE uc.user_id IN(SELECT value::bigint FROM jsonb_array_elements_text(${idsJson}::jsonb)) AND uc.owned=true`;
