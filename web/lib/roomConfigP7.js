...
function initial(config={}){return{maxPlayers:clampInt(3,10,config.maxPlayers,6),pointsToWin:clampInt(3,20,config.pointsToWin,10),handSize:clampInt(5,15,config.handSize,5),useStandardDeck:config.useStandardDeck!==false,cardCreationEnabled:config.cardCreationEnabled!==false,playerCardsEnabled:config.playerCardsEnabled!==false,afkEnabled:config.afkEnabled!==false,buffsEnabled:config.buffsEnabled===true};}
function apply(room,playerId,partial={}){
 ...
 if(!PRE_START.has(room.state)||room.rewardConfigSnapshot)throw new Error('As regras econômicas não podem ser alteradas depois que a partida começou.');
...
function publicConfig(room){return{maxPlayers:room.maxPlayers,pointsToWin:room.pointsToWin,handSize:room.handSize,useStandardDeck:room.useStandardDeck!==false,cardCreationEnabled:room.cardCreationEnabled!==false,playerCardsEnabled:room.playerCardsEnabled!==false,afkEnabled:room.afkEnabled!==false,buffsEnabled:room.buffsEnabled===true,rewardConfigSnapshot:room.rewardConfigSnapshot||null};}
