...
function blackGapCount(text){return (String(text||'').match(/_+/gu)||[]).length;}
function whiteCardsRequiredForBlack(text){return blackGapCount(text)>=2?2:1;}
function normalizeBlackCardDisplay(text,{requireGap=true,maxGaps=MAX_BLACK_GAPS}={}){
 const cleaned=cleanDisplayText(text),gaps=blackGapCount(cleaned);
 if(requireGap&&gaps<1)throw new Error('Toda Carta Preta precisa ter pelo menos uma lacuna (_).');
 if(Number.isFinite(maxGaps)&&gaps>maxGaps)throw new Error(`Carta Preta pode ter no máximo ${maxGaps} lacunas.`);
...
module.exports={BLACK_GAP,MAX_BLACK_GAPS,canonicalCardType,legacyCardType,cleanDisplayText,blackGapCount,whiteCardsRequiredForBlack,normalizeBlackCardDisplay,prepareDisplayText,normalizeCardText,canonicalIdentity,sameCanonicalCard,creationKind,submissionIsCreation};
