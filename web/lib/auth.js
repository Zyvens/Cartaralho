...
function createRecoveryCode(){return crypto.randomBytes(8).toString('hex').toUpperCase();}
...
async function getUserFromRequest(req){if(req.authUser)return req.authUser;const token=readBearer(req);if(!token)return null;const rows=await sql`SELECT u.id,u.username,u.display_name,u.email,u.avatar_data,u.bio,u.created_at FROM auth_sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=${tokenHash(token)} AND s.expires_at>now() LIMIT 1`;return rows[0]||null;}
