'use strict';
const c=require('./marketplaceCommon'),prestige=require('./prestigeService');
const BEST_WORLD_RANK_VERSION='best-world-v1';
async function getState(userId){
 const id=c.uid(userId),b=await c.balances(id);
 const[catalog,purchases,packCards,ledger,buffInventory,prestigeState,gate]=await Promise.all([
  ...
  prestige.profileState(id),prestige.marketGate(id)
 ]);
 return{catalog,purchases,packCards,ledger,buffInventory,cosmeticOwnerships:prestigeState.ownerships,specialEntitlements:prestigeState.entitlements,prestigeTitles:prestigeState.titles,prestigeFrames:prestigeState.frames,userLevel:gate.level,cosmeticMinimumLevel:gate.minimumLevel,cosmeticEligible:gate.eligible,...b,rankVersion:BEST_WORLD_RANK_VERSION,buffsFeatureEnabled:process.env.BUFFS_FEATURE_ENABLED!=='false',cosmeticsFeatureEnabled:process.env.COSMETICS_FEATURE_ENABLED!=='false'};
}
