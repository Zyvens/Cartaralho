...
const{canonicalIdentity,normalizeBlackCardDisplay,cleanDisplayText}=require('./cardIdentity');
...
async function create({userId,type,text,matchId,creatorName,creationId}){
 const cardType=normalizeType(type);if(!cardType)return{status:'invalid_type'};
 const prepared=cardType==='black'?normalizeBlackCardDisplay(text,{requireGap:true}):cleanDisplayText(text),identity=canonicalIdentity(cardType,prepared);
 const rows=await sql`SELECT create_paid_player_card(${Number(userId)},${cardType},${identity.normalizedText},${identity.displayText},${String(matchId||'')},${String(creatorName||'')},${String(creationId||'')}) result`;
 return{...cleanResult(rows[0]?.result),displayText:identity.displayText};
}
