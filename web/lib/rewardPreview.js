...
function preview({pointsToWin=10,participants=6,handSize=5}={}){
 const points=Math.round(rewardRules.clamp(3,20,pointsToWin)),players=Math.round(rewardRules.clamp(3,10,participants)),hand=Math.round(rewardRules.clamp(5,15,handSize));
 const curve=rewardRules.rewardCurve(points,players),klass=matchClass(curve.effort);
 const payout=position=>rewardRules.payoutForPosition(position,players,curve,true);
