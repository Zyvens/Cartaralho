...
test('curva econômica mantém referências v1',()=>{
 const cases=[
  [3,3,[8,4,2],0],[5,6,[42,21,11],0],[10,3,[71,35,19],0],[10,6,[150,75,40],0],
  [10,10,[260,130,69],37],[15,6,[314,157,84],55],[15,10,[545,273,145],132],
  [20,6,[531,266,142],127],[20,10,[921,461,246],257]
 ];
...
test('payout soma colocação, sobrevivência e consolação',()=>{
 const c=rewards.rewardCurve(20,10);
 assert.deepEqual(rewards.payoutForPosition(1,10,c,true),{placement:921,survival:257,consolation:0,total:1178});
 assert.deepEqual(rewards.payoutForPosition(10,10,c,true),{placement:0,survival:257,consolation:1,total:258});
 assert.deepEqual(rewards.payoutForPosition(10,10,c,false),{placement:0,survival:0,consolation:1,total:1});
});
