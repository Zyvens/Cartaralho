...
test('Perfil e Sair não herdam o marcador mobile legado que escondia seus spans',()=>{
 ...
 assert.match(currentCss,/p56-account-action-icon[\s\S]*display:grid!important/);
 assert.match(currentCss,/@media\(max-width:620px\)[\s\S]*p56-account-action-copy[\s\S]*display:none!important/);
});

test('CSS P73 foi absorvido pelo owner visual atual e o arquivo histórico não possui regras',()=>{
 assert.match(currentCss,/home-account-balance[\s\S]*display:inline-flex!important/);
