...
test('catálogo contém exatamente 7 molduras e 6 títulos com preços v1.4',()=>{const rows=[['Fita Isolante Premium',25000],['Ouro de Pobre',40000],['Neon de Procedência Duvidosa',55000],['Glitch Radioativo',75000],['Buraco Negro Fiscal',100000],['Moldura Agiota',150000],['Lavagem Completa',250000],['Cliente Preferencial',20000],['Lavador de Moedinhas',30000],['Patrocinador do Caos',45000],['Dinheiro Não Compra Talento',65000],['Herdeiro do Cartaralho',100000],['Patrimônio Inexplicável',200000]];for(const[name,price]of rows){assert.ok(migration.includes(`'${name}'`),name);assert.ok(migration.includes(`,${price},`),`${name}:${price}`);}assert.equal((migration.match(/\('cosmetic_(?:frame|title)_/g)||[]).length,13);});
...
test('molduras cosméticas permanecem separadas dos tiers de progressão',()=>{assert.doesNotMatch(migration,/Copper|Silver|Gold|Platinum|card_personal_progress|user_card_progress/);assert.ok(Object.values(defs.COSMETIC_EQUIP_KEYS).filter(x=>x.type==='frame').length===7);});
