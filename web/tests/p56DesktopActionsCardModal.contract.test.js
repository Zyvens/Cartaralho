...
test('Ficha elimina overlays concorrentes e preserva progressão e origem',()=>{assert.match(cards,/p41-card-detail-overlay/);assert.match(cards,/p55-card-detail-overlay/);assert.match(cards,/p56-card-detail-overlay/);assert.match(detailCss,/\.p56-card-detail-overlay[\s\S]*z-index:65000/);assert.match(detailCss,/prefers-reduced-motion/);assert.match(progression,/p56-progress-metric/);assert.match(progression,/MESAS VISITADAS/);assert.match(progression,/PESSOAS QUE POSSUEM/);});
