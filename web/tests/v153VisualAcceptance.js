...
(async()=>{
 ...
 try{
  ...
  const rewards=await page.evaluate(()=>{const row=document.querySelector('#mission-card .mission-row'),coin=row?.querySelector('.p52-mission-coin-pill'),xp=row?.querySelector('.mission-xp-pill'),buff=row?.querySelector('.p10-mission-buff');if(!row||!coin||!xp||!buff)return null;const c=coin.getBoundingClientRect(),x=xp.getBoundingClientRect(),b=buff.getBoundingClientRect(),cs=getComputedStyle(buff);return{coinTop:c.top,xpTop:x.top,buffTop:b.top,buffText:buff.textContent,paddingLeft:parseFloat(cs.paddingLeft)||0,paddingRight:parseFloat(cs.paddingRight)||0,rowWidth:row.getBoundingClientRect().width,rewardsRight:b.right-row.getBoundingClientRect().right};});
  assert('mission reward geometry available',!!rewards,JSON.stringify(rewards));
  assert('coin XP and BUFF share one reward row',Math.max(rewards.coinTop,rewards.xpTop,rewards.buffTop)-Math.min(rewards.coinTop,rewards.xpTop,rewards.buffTop)<=3,JSON.stringify(rewards));
