...
const cosmetic=read('public/js/cosmeticUI.js'),prestige=read('public/js/prestigeUI.js'),css=read('public/css/prestigeP11.css'),html=read('public/index.html');
...
test('P11 não introduz MutationObserver amplo novo',()=>{assert.doesNotMatch(cosmetic,/MutationObserver/);assert.doesNotMatch(prestige,/MutationObserver/);});

test('render global registra nomes e raridades de títulos P11',()=>{for(const key of['cliente-preferencial','lavador-de-moedinhas','patrocinador-do-caos','dinheiro-nao-compra-talento','herdeiro-do-cartaralho','patrimonio-inexplicavel','o-criador','betinha'])assert.ok(prestige.includes(`'${key}'`),key);assert.match(prestige,/MetaTitleNames/);assert.match(prestige,/MetaTitleRarities/);assert.match(prestige,/decorateTitles/);assert.match(prestige,/title-rarity-/);});
...
test('assets P11 carregam depois das dependências existentes',()=>{assert.ok(html.indexOf('achievementsP10.css')<html.indexOf('prestigeP11.css'));assert.ok(html.indexOf('achievementUI.js')<html.indexOf('prestigeUI.js'));assert.ok(html.indexOf('marketplaceUI.js')<html.indexOf('cosmeticUI.js'));});
