...
test('P59 carrega depois de P58 como camada visual preservada',()=>{
...
test('P59 permanece registrado após releases posteriores',()=>{
 assert.match(release,/APP_VERSION='v1\.4\.59'/);
 assert.match(version,/releaseP(?:59|[6-9]\d)/);
 assert.match(notifications,/releaseP(?:59|[6-9]\d)/);
 assert.match(notifications,/P59_RELEASE|releaseP59/);
