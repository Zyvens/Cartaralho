...
test('contribuição mínima é uma carta e zero exige confirmação',()=>{
...
test('Pronto sem contribuição exige confirmação e resposta/broadcast são compactos para evitar 413',()=>{
 assert.match(readyApi,/CONTRIBUTION_CONFIRMATION_REQUIRED/);assert.match(readyApi,/acceptNoContribution/);assert.match(readyApi,/playerStatuses/);assert.doesNotMatch(readyApi,/ok\(res,\{[^}]*players,/s);assert.match(readyApi,/avatarData em base64/);
 assert.match(auth,/err\.code=data\.code/);assert.match(lobby,/Mão de Vaca/);assert.match(lobby,/não poderá coletar Espólio/);
});
...
test('P18 CSS e Central são carregados depois dos pacotes anteriores',()=>{assert.ok(index.indexOf('css/p18.css')>index.indexOf('css/p17.css'));assert.ok(index.indexOf('js/notificationsUI.js')>index.indexOf('js/refinementP13.js'));});
