...
SELECT points_to_win,participants_at_start,class_key,COUNT(*) matches,ROUND((AVG(duration_seconds)/60.0)::numeric,1) avg_minutes,ROUND((percentile_cont(.5) WITHIN GROUP(ORDER BY duration_seconds)/60.0)::numeric,1) median_minutes,ROUND((percentile_cont(.9) WITHIN GROUP(ORDER BY duration_seconds)/60.0)::numeric,1) p90_minutes FROM match_telemetry WHERE finished_at IS NOT NULL AND duration_seconds>0 GROUP BY points_to_win,participants_at_start,class_key ORDER BY points_to_win,participants_at_start;
...
SELECT COUNT(*) wallets,ROUND((percentile_cont(.25) WITHIN GROUP(ORDER BY balance))::numeric,0) p25,ROUND((percentile_cont(.50) WITHIN GROUP(ORDER BY balance))::numeric,0) median,ROUND((percentile_cont(.75) WITHIN GROUP(ORDER BY balance))::numeric,0) p75,ROUND((percentile_cont(.90) WITHIN GROUP(ORDER BY balance))::numeric,0) p90,MAX(balance) max_balance FROM dirty_coin_wallets;
-- 6) Tempo até primeira compra.
WITH first_purchase AS(SELECT user_id,MIN(created_at) first_at FROM market_purchases GROUP BY user_id) SELECT COUNT(*) buyers,ROUND(AVG(EXTRACT(EPOCH FROM(fp.first_at-u.created_at))/3600.0)::numeric,1) avg_hours_to_first_purchase,ROUND((percentile_cont(.5) WITHIN GROUP(ORDER BY EXTRACT(EPOCH FROM(fp.first_at-u.created_at))/3600.0))::numeric,1) median_hours FROM first_purchase fp JOIN users u ON u.id=fp.user_id;
