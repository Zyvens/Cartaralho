(()=>{
  ...
  if(window.MetaUI){
    MetaUI.missionRow=function(m){
      ...
      return `<div class="mission-row ${m.completed?'done':''}"><b>${m.completed?'✅':'🎯'} ${escText(m.name)}</b><small class="mission-copy">${escText(m.description)}</small><span class="mission-xp-pill">+${Number(m.xp||0)} XP</span><div class="mission-progress"><span style="width:${pct}%"></span></div><small class="mission-count">${Number(m.progress||0)}/${Number(m.target||0)}</small></div>`;
    };
  }
