(()=>{
 ...
 HomeScreen.renderStats=async panel=>{await previousStats(panel);const d=await economy();if(!d?.economyUiEnabled||!d.economy)return;const root=panel.querySelector('.profile-panel');if(!root)return;const labels={starter_grant:'Bônus inicial',match_placement:'Colocação',match_survival:'Sobrevivência',match_consolation:'Consolação',mission_reward:'Missão',clean_card_purchase:'Compra de Carta Limpa',adjustment:'Ajuste'};const section=document.createElement('div');section.className='meta-section';section.innerHTML=`<h4>🪙 Moedas Sujas</h4><div class="xp-card"><b>Saldo: ${fmt(d.economy.balance)}</b><small>Ledger auditável · saldo ligado à conta</small></div><div class="recent-matches">${(d.economy.ledger||[]).slice(0,10).map(x=>{const amount=Number(x.amount||0);return`<div><b>${amount>=0?'+':''}${fmt(amount)} · ${labels[x.transaction_type]||x.transaction_type}</b><span>${new Date(x.created_at).toLocaleString('pt-BR')}</span></div>`;}).join('')||'<p>Nenhuma movimentação.</p>'}</div>`;root.insertBefore(section,root.children[1]||null);};
