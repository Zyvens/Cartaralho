(()=>{
 ...
 HomeScreen.renderCards=async panel=>{
  ...
  panel.querySelectorAll('.profile-card[data-card-id]').forEach(el=>{
   ...
   const small=el.querySelector('small');if(small)small.textContent=`${p.material.label} · borda ${p.border.label} · ${fmt(p.whitePersonalWins)} vitórias suas · ${fmt(p.worldHolders)} proprietários no mundo`;
   el.onclick=e=>{if(e.target.closest('.favorite-card'))return;const g=p.global||{},original=p.isOriginal?'<br><br><strong>🧬 CARTA ORIGINAL</strong><br>Você é um dos criadores originais desta identidade canônica.':'';Modal.show({title:escape(card.text),message:`<strong>Fundo: ${escape(p.material.label)}</strong><br>Rodadas que você venceu utilizando esta carta: ${fmt(p.whitePersonalWins)}.<br>Próximo: ${escape(p.material.nextLabel||'máximo')}${p.material.nextLabel?` · faltam ${fmt(p.material.remaining)}`:''}.<br><br><strong>Borda: ${escape(p.border.label)}</strong><br>Jogadores distintos que possuem esta Carta Canônica no mundo: ${fmt(p.worldHolders)}.<br>Aquisições por Espólio aumentam esta circulação automaticamente.<br>Próximo: ${escape(p.border.nextLabel||'máximo')}${p.border.nextLabel?` · faltam ${fmt(p.border.remaining)}`:''}.<br><br><strong>Legado global: ${escape(g.legacyLabel||labelLevel(g.legacyLevel))}</strong><br>Alcance: ${fmt(g.reach)} proprietários · adoções por Espólio: ${fmt(g.adoptions)} · presença: ${fmt(g.presence)} partidas · vitórias globais: ${fmt(g.wins)} · coincidências criativas: ${fmt(g.creativeCoincidences)}.${original}`,confirmText:'Fechar'});};
  });
 };
 ...
 SocketClient.on('round_result',data=>{const plays=Array.isArray(data?.originalPlays)?data.originalPlays:[];if(plays.length){const names=[...new Set(plays.map(x=>x.nickname).filter(Boolean))];setTimeout(()=>Toast.info(`🧬 DIRETO DA FONTE · ${names.slice(0,2).join(' e ')}${names.length>2?` e +${names.length-2}`:''} jogou uma criação original.`),180);}if(data?.originalCelebration)setTimeout(()=>Toast.success(`${data.originalCelebration.message} · ${data.originalCelebration.detail}`),520);});
})();
