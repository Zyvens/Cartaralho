...
(()=>{
 ...
 const Detail={
  ...
  open(c){
   ...
   overlay.innerHTML=`<section class="p55-card-detail-shell" role="dialog" aria-modal="true" aria-label="Detalhes da carta"><header class="p55-card-detail-header"><div><span>DETALHES DA CARTA</span><h2>Evolução e origem</h2></div><button type="button" class="p55-card-detail-close" aria-label="Fechar">✕</button></header><main class="p55-card-detail-body"><section class="p55-card-detail-hero"><div class="p55-card-preview-host"></div><div class="p55-card-detail-summary"><div class="p55-card-detail-pills"><span>${black?'🖤 Carta Preta':'🤍 Carta Branca'}</span>${c.is_player_card?'<span>★ De Jogador</span>':''}${c.is_favorite?'<span>⭐ Favorita</span>':''}${c.is_native?'<span>🎴 Nativa</span>':''}</div><h3>${esc(c.text)}</h3><p>A evolução desta carta acompanha o uso real nas partidas e as recriações feitas por outros jogadores.</p></div></section><section class="p55-evolution-grid">${this.progression('material',c)}${this.progression('border',c)}</section><section class="p55-origin-card"><div class="p55-origin-title"><span>ORIGEM</span><h3>De onde veio esta carta</h3></div><div class="p55-origin-grid"><div><small>CRIADA POR</small><b>${esc(c.is_native?'Cartaralho':o.creatorName||'Origem não identificada')}</b></div><div><small>MESAS VISITADAS</small><b>${Number(o.tablesVisited||0)}</b></div><div><small>PESSOAS QUE POSSUEM</small><b>${Number(o.holders||0)}</b></div>${o.firstRoomCode?`<div><small>PRIMEIRA MESA</small><b>${esc(o.firstRoomCode)}</b></div>`:''}</div>${o.originUncertain?'<p class="p55-origin-note">Alguns dados de origem foram recuperados de registros antigos e podem estar incompletos.</p>':''}</section></main></section>`;
