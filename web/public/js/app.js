...
const App = {
  ...
  init() {
    ...
    // Online build: every visitor is a normal player on the shared cloud
    // room, there's no "one PC hosts on the LAN, others are guests"
    // concept here — that only applied to the desktop app's localtunnel
    // sharing feature (kept for parity, but it can never match on a
    // vercel.app / custom domain).
