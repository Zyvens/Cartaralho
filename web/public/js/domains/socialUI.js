...
(()=>{
 ...
 CartDomains.claim('socialUI','domains/socialUI.js',()=>{
  ...
  function installSocialRenderer(){if(!window.SocialUI||SocialUI.__domainPresence)return;SocialUI.__domainPresence=true;const baseFriends=SocialUI.renderFriends.bind(SocialUI);SocialUI.renderFriends=async function(root,...args){const out=await baseFriends(root,...args);try{lastList=await list();}catch(_){ }ensureFriendPill();return out;};}
