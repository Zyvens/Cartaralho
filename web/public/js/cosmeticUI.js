...
const C={
 ...
 card(p,m,owned,locked){
  ...
  return `<article class="market-product cosmetic-product rarity-${m.esc(rarity)} ${ownedNow?'cosmetic-owned':''}">
   <div class="cosmetic-product-top"><small>${c.cosmeticType==='frame'?'MOLDURA COSMÉTICA':'TÍTULO COMPRÁVEL'}</small><span class="cosmetic-rarity rarity-${m.esc(rarity)}">${m.esc(({rare:'Incomum',superrare:'Raro',epic:'Épico',legendary:'Lendário',celestial:'Celestial'})[rarity]||rarity)}</span></div>
 ...
 render(body,m){
  const products=(m.data.catalog||[]).filter(x=>x.category==='cosmetic'),frames=products.filter(x=>x.product_kind==='cosmetic_frame'),titles=products.filter(x=>x.product_kind==='cosmetic_title'),owned=ownedSet(m),level=Number(m.data.userLevel||1),min=Number(m.data.cosmeticMinimumLevel||5),locked=!m.data.cosmeticEligible;
  body.innerHTML=`<section class="cosmetic-hero"><div><small>PRESTÍGIO DUVIDOSO</small><h3>Cosméticos</h3><p>Itens permanentes, absurdamente caros e mecanicamente inúteis — como todo símbolo de status respeitável.</p></div><div class="cosmetic-level ${locked?'locked':'ready'}"><span>NÍVEL</span><b>${level}</b><small>${locked?`🔒 Compras liberadas no nível ${min}`:'✓ Compras liberadas'}</small></div></section>${locked?`<div class="cosmetic-lock-note"><b>🔒 Disponível no nível ${min}</b><span>As prévias já estão liberadas. Jogue e acumule XP para comprar.</span></div>`:''}<section class="market-section"><div class="market-section-title"><div><h3>◉ Molduras de Ostentação</h3><p>Cosméticos de avatar. Não substituem nem avançam as molduras Copper, Silver, Gold e Platinum de progressão.</p></div></div><div class="market-grid cosmetic-grid">${frames.map(p=>this.card(p,m,owned,locked)).join('')}</div></section><section class="market-section"><div class="market-section-title"><div><h3>🏷️ Títulos Compráveis</h3><p>Achievement não está à venda. Aqui ficam somente títulos explicitamente marcados como cosméticos.</p></div></div><div class="market-grid cosmetic-grid">${titles.map(p=>this.card(p,m,owned,locked)).join('')}</div></section><section class="cosmetic-special-note"><b>✦ Prestígio especial</b><span><strong>O Criador</strong> e <strong>Betinha</strong> nunca aparecem à venda: são entitlements especiais.</span></section>`;
