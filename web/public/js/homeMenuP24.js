...
(()=>{
 ...
 function apply(){
  ...
  ORDER.forEach((sel,index)=>{const node=actions.querySelector(sel);if(!node)return;node.style.setProperty('order',String(index+1),'important');setLabel(node,LABELS[sel]);});
 }
