const AdminAuth = {
  ...
  header() {
    if (!this.password) {
      this.password = sessionStorage.getItem('cartalho_admin_pw') || window.prompt('Senha de administrador:') || '';
      sessionStorage.setItem('cartalho_admin_pw', this.password);
    }
    return 'admin:' + this.password;
  },

  /** Called when the server rejects the current password, so the next request re-prompts. */
  reset() {
    this.password = null;
    sessionStorage.removeItem('cartalho_admin_pw');
  },
};
  ...
  async loadDeck() {
    try {
      const response = await fetch('/api/admin/deck', {
        headers: { 'Authorization': AdminAuth.header() }
      });
      if (!response.ok) { AdminAuth.reset(); throw new Error('Não autorizado'); }
        ...
        try {
          const response = await fetch('/api/admin/deck', {
            ...
            headers: {
              ...
              'Authorization': AdminAuth.header()
            },
