...
module.exports=withErrors(async(req,res)=>{
 ...
 if(req.method==='GET'){
  ...
  if(!room.players.has(String(user.id)))return fail(res,403,'Você não participa desta sala.');
 ...
 const room=await roomStore.loadRoom(code);if(!room)return fail(res,404,'Sala não encontrada.');if(!room.players.has(String(user.id)))return fail(res,403,'Você não participa desta sala.');
 if(action==='resolve_mao_de_vaca'){
  ...
  await broadcast(room.code,'buff_resolved',{buffKey:'buff_mao_de_vaca',userId:user.id,nickname:room.players.get(String(user.id))?.nickname||user.display_name,message:'🐄 Mão de Vaca resolvida: duas cartas devolvidas.'});
 ...
 const def=definitions.get(buffKey),actor=room.players.get(String(user.id));if(!r.replayed&&r.publicMessage)await broadcast(room.code,'buff_activated',{buffKey,name:def?.name||buffKey,icon:def?.icon||'⚡',userId:user.id,nickname:actor?.nickname||user.display_name,targetUserId:r.effect?.targetUserId||null,targetNickname:r.effect?.targetNickname||null,message:r.publicMessage});
