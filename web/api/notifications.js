...
const{APP_VERSION,RELEASE}=require('../lib/releaseP39');
...
module.exports=withErrors(async(req,res)=>{
 ...
 const blocked=new Set([RELEASE.id,P38_RELEASE.id,P37_RELEASE.id]);
 data.updates=[RELEASE,P38_RELEASE,P37_RELEASE,...(data.updates||[]).filter(x=>!blocked.has(x.id))];
 ok(res,data);
});
