...
const progression=require('../../lib/cardProgressionService');
...
module.exports=withErrors(async(req,res)=>{
 ...
 const cards=rows.map(r=>{
  const materialScore=Number(r.white_personal_wins||0)+Number(r.black_personal_uses||0);
  const borderScore=ownedDistinctCards;
  ...
  return{
   ...
   whitePersonalWins:Number(r.white_personal_wins||0),
   ...
   global:{reach:Number(r.reach_count||0),adoptions:Number(r.adoption_count||0),creativeCoincidences:Number(r.creative_coincidence_count||0),presence:Number(r.presence_count||0),uses:Number(r.total_uses||0),wins:Number(r.global_wins||0),legacyScore:score,legacyLevel:level.key,legacyLabel:level.label}
  };
 });
 ok(res,{progressionEnabled:process.env.CARD_PROGRESSION_V2_ENABLED!=='false',ownedDistinctCards,cards,thresholds:{material:progression.MATERIAL_THRESHOLDS,border:progression.BORDER_THRESHOLDS,legacy:progression.LEGACY_THRESHOLDS}});
});
