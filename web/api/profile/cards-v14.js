...
const progression=require('../../lib/cardProgressionRules');
...
function borderState(score){const p=progression.progressFor(Number(score||0),'border');return{tier:p.tier,progress:{current:p.score,nextTier:p.nextTier,target:p.target,remaining:p.remaining}};}
...
module.exports=withErrors(async(req,res)=>{
 ...
 for(const row of rows){
  ...
  if(byKey.has(key)){
   const target=byKey.get(key);target.is_favorite=Boolean(target.is_favorite||row.is_favorite);target.owned=Boolean(target.owned||row.owned);target.times_used+=Number(row.times_used||0);target.matches_used+=Number(row.matches_used||0);target.times_seen+=Number(row.times_seen||0);target.times_won+=Number(row.times_won||0);target.duplicate_creation_count+=Number(row.duplicate_creation_count||0);target.origin.tablesVisited=Math.max(target.origin.tablesVisited,Number(row.tables_visited||0));continue;
  }
  ...
  const border=borderState(meta?.external_presence_matches||0);
  const card={...row,text:meta?.display_text||row.text,canonicalCardId:meta?.id||null,isOriginal,times_used:Number(row.times_used||0),matches_used:Number(row.matches_used||0),times_seen:Number(row.times_seen||0),times_won:Number(row.times_won||0),duplicate_creation_count:Number(row.duplicate_creation_count||0),externalPresenceMatches:Number(meta?.external_presence_matches||0),borderTier:border.tier,borderProgress:border.progress,origin:{authors:authors.map(a=>({userId:a.user_id,displayName:a.display_name,username:a.username||null,authoredAt:a.authored_at})),creatorUserId:authors[0]?.user_id||row.legacy_creator_user_id||null,creatorName,creatorUsername:authors[0]?.username||row.legacy_creator_username||null,firstRoomCode:meta?.origin_match_id||row.legacy_first_room_code||null,firstSeenAt:meta?.created_at||row.legacy_first_seen_at||null,originUncertain:Boolean(meta?.origin_uncertain),tablesVisited:Number(row.tables_visited||0),holders:Number(meta?.holders||0),adoptions:Number(meta?.adoptions||0),recreatedCount:Number(row.legacy_recreated_count||0)},genealogy:{acquisitionSource:meta?.acquisition_source||'legacy_import',sourceUserId:meta?.source_user_id||null,sourceUserName:meta?.source_display_name||null,sourceUsername:meta?.source_username||null,sourceMatchId:meta?.source_match_id||null,acquiredAt:meta?.acquired_at||row.created_at||null}};
  byKey.set(key,card);merged.push(card);
 }

 ok(res,{cards:merged.map(c=>({...c,
  ...
  rarityExplanation:{
   ...
   border:'O contorno mede circulação externa encontrada por você: quando outro jogador traz esta mesma Carta Canônica para uma partida em que você participa, a partida conta uma vez. Uso próprio não conta.'
  }
 }))});
});
