...
module.exports=withErrors(async(req,res)=>{
 ...
 const{code,type,text,creationId,libraryMode}=getBody(req);
 if(String(creationId||'').trim().length<8)return fail(res,400,'Identificador de criação inválido.');
 let matchId='',creatorName=user.display_name||user.username||'Jogador';
 if(!libraryMode){
  ...
  matchId=room.code;creatorName=player.nickname||creatorName;
 }
 ...
 let result;
 try{result=await cleanCards.create({userId:user.id,type:cardType,text:display,matchId,creatorName,creationId});}
 ...
 if(result.status==='duplicate_owned')return fail(res,409,'Você já possui essa Carta de Jogador.');
 ...
 ok(res,{message:'Carta sujada com sucesso.',card:{type:cardType==='black'?'blackCards':'whiteCards',text:result.displayText,canonicalCardId:result.canonicalCardId,creationKind:result.creationKind||null},inventory:result,libraryMode:!!libraryMode});
});
