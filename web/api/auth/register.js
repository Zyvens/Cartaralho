...
module.exports = withErrors(async (req, res) => {
  ...
  if (!/^[a-z0-9_.-]{3,24}$/.test(normalized)) return fail(res, 400, 'Usuário deve ter 3 a 24 caracteres: letras, números, ponto, hífen ou _.');
  ...
  const rows = await sql`INSERT INTO users (username, display_name, password_hash) VALUES (${normalized}, ${name}, ${passwordHash}) RETURNING id, username, display_name, created_at`;
  const user = rows[0];
  await sql`INSERT INTO user_cards (user_id,type,text,owned)
            SELECT ${user.id}, type, text, true FROM deck_cards WHERE is_hidden=false
