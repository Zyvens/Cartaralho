 ...
 * @returns {{ valid: boolean, error?: string }}
 */
function validateRoomJoin(room, nickname, ip, sessionId) {
  if (!room) {
  ...
  // Verifica se é um reingresso
  for (const [oldSocketId, player] of room.players) {
    if (!player.connected) {
      const matchIp = ip && player.ip === ip;
      const matchSession = sessionId && player.sessionId === sessionId;
      const matchNickname = player.nickname.trim().toLowerCase() === trimmedNickname;

      if (matchSession || matchIp || matchNickname) {
        return { valid: true, isRejoin: true, oldSocketId };
      }
    }
  }
